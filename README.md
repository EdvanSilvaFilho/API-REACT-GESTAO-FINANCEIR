# APLICAÇÃO GESTÃO FINANCEIRA | Front-end

Após alguns meses trabalhando em projetos mais simples, me senti impulsionado a desenvolver uma aplicação que irá auxiliar no processo de gerenciamento de finanças, garantindo organização, planejamento e balanço diário refletido de forma atualizada. 


####  FUNCIONALIDADES:

- Cadastro de uma nova transação (**cadastre por categoria, valor, descrição e tipo(crédito/débito)**)
- Edição de uma transação (**edite todos os campos, ou somente alguns**)
- Exclusão de uma transação (**selecione uma transação para excluir por vez**)
- Listagem de transações (**Olista todas as transações**)
- Ordernação dos items conforme clicado no header da tabela nas seguintes colunas: Data, Dia da semana e Valor (**alterne as transações entre ascendente e descentende**)
- Na parte de resumo, o valor de entradas,saídas e saldo é referente ao que está listado na tabela 
- Filtros (selecione um ou mais), veja abaixo:
  - Dia da semana (**filtre as transações por dia da semana**)
  - Categoria (**filtre as transações por categorias**)
  - Valor (**filtre as transações por valor**)
...
Quando você fizer todos esse procedimentos, o projeto em `React` irá executar e exibir no seu navegador a aplicação.
...
Após realizar as validações, após o usuário clicar no botão **confirmar**, uma nova transação deve será inserida e a tabela de listagem deve será atualizada.
...
Ao clicar no ícone de editar uma transação, o modal (que foi utilizado para adicionar uma nova transação) será aberto e as informações da transação "clicada", deverão ser preenchidas automaticamente, assim como a imagem abaixo:

![](https://i.imgur.com/EyRegfx.png)

Após validar os campos e o usuário clicar em confirmar, a transação será ser atualizada na `API` usando requisição do tipo `patch` ou `put` e a transação deverá ser atualizada na tabela de listagem de transações.
...
2. **Dia da semana**, nessa coluna utilizamos apenas os primeiros nomes dos dias da semana, ao invéz de Segunda-Feira, utilizaremos o formato `Segunda`.
...
A área de filtros por padrão é oculta, por isso você deve clicar no botão **Filtrar** a área de filtro será exibida e quando clicar novamente será ocultada, veja abaixo o botão que exibe/oculta a área de filtros:
